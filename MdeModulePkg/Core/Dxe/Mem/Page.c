  ...
  )
{
  ...
  //
  CoreNotifySignalList (&gEfiEventMemoryMapChangeGuid); // OSDDEBUG Need to revist this, should every gcd update trigger this, is locking still correct?
  ...
  mMapStack[mMapDepth].ImageHandle  = ImageHandle;
  ...
  while (Start < End) {
    ...
    for (Link = mGcdMemorySpaceMap.ForwardLink; Link != &mGcdMemorySpaceMap; Link = Link->ForwardLink) {
      ...
      if ((Entry->BaseAddress <= Start) && (Entry->EndAddress > Start)) {
        ...
        EfiGcdMemoryType = Entry->GcdMemoryType;
    ...
    if (ChangingType) {
      ...
      // if we are changing type, pick the right GCD mem type
      switch (NewType) {
        ...
        case EfiMemoryMappedIOPortSpace:
          EfiGcdMemoryType = EfiGcdMemoryTypeMemoryMappedIo;
          break;
        case EfiPersistentMemory:
          EfiGcdMemoryType = EfiGcdMemoryTypePersistent;
          break;
        case EfiUnacceptedMemoryType:
        case EfiUnusableMemory:
          EfiGcdMemoryType = EFI_GCD_MEMORY_TYPE_UNACCEPTED;
          break;
        default:
          // any other type mark it system mem
  ...
  )
{
  ...
  for ( ; MemoryMap != MemoryMapDescriptor; MemoryMap = NEXT_MEMORY_DESCRIPTOR (MemoryMap, DescriptorSize)) {
    ...
    if (MemoryMap->PhysicalStart + EFI_PAGES_TO_SIZE ((UINTN)MemoryMap->NumberOfPages) == MemoryMapDescriptor->PhysicalStart) {
      ...
      //
      // Return MemoryMapDescriptor as the next available slot in the MemoryMap array
      //
  ...
  )
{
  ...
  for (Link = mGcdMemorySpaceMap.ForwardLink; Link != &mGcdMemorySpaceMap; Link = Link->ForwardLink) {
    ...
    // per the UEFI spec, all of memory is included in the EFI_MEMORY_MAP except for non-existent memory
  ...
  )
{
  ...
  if (Start == 0) {
    ...
    if (NeedGuard) {
      Status = CoreConvertPagesWithGuard (Start, NumberOfPages, PoolType);
      if (EFI_ERROR (Status)) {
        Start = 0;
      }
    } else {
